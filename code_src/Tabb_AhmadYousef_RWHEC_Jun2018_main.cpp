...
int main(int argc, char ** argv) {
  ...
  */
  ...
  filename = source_dir + "/cam_0.txt";
  FILE * file;
  file = fopen(filename.c_str(), "r");
  if (!file) {
    cout << "Camera0 file is null -- fix and exiting for now... " << filename << endl;
    exit(1);
  } else {
    fclose(file);
    ReadRobotFileRobotCaliTxt(filename, Atemp);
  }
  ...
  for (int i = 0; i < Bs.size(); i++) {
    if (inverseAB) {
      Bs[i] = ConvertToMM(Bs[i]).inverse().eval();
    } else {
  ...
  // key variables
  PARAM_TYPE param_type = AxisAngle;
  COST_TYPE cost_type = c1;
...
int RobotWorldHandEyeCalibration(double square_mm_height, double square_mm_width,
  ...
  if (!do_rwhec && !do_reconstruction) {
    ...
    if (!do_camcali) {
      ...
      for (int k = 0; k < robot_mounted_cameras; k++) {
        /////////////////////// Camera calibration ....///////////////////////////////////////////
        filename = write_dir + "/camera_results" + ToString < int > (k) + "/program_readable_details.txt"; in .open(filename.c_str());
  ...
  for (int option = 0; option < 18; option++) {
    ...
    if (do_reconstruction) {
      ...
      string trans_file = write_result_directory + "/transformations.txt"; in .open(trans_file.c_str());
...
string FindValueOfFieldInFile(string filename, string fieldTag, bool seperator) {
  ...
  cout << "Found! " << found << " field " << fieldTag << " and result " << return_str << endl; in .close();
